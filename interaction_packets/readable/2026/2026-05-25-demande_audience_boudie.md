---
title: "Interaction lisible — Demande d'audience / processus constitutionnel Corse (Florent Boudié)"
description: "Human-readable copy (disclosure D3) of the 2026-05-25 official audience request to the rapporteur of the Corsican constitutional bill, on Autonomie de Capacité; no reply detected so far."
author: "Jean Hugues Noël Robert, baron Mariani"
affiliation: "Institut Mariani — émanation R&D de C.O.R.S.I.C.A., 1 cours Paoli, F-20250 Corte, Corsica"
date: "2026-05-25"
status: "journal"
license: "CC BY 4.0"
canonical_url: https://github.com/JeanHuguesRobert/JeanHuguesRobert/blob/main/interaction_packets/readable/2026/2026-05-25-demande_audience_boudie.md
last_stamped_at: 2026-06-01
---

# Interaction lisible — Demande d'audience / processus constitutionnel Corse (Florent Boudié)

## Identification

- **ID** : `2026-05-25-001`
- **Date d'envoi** : 2026-05-25
- **Date de réponse** : aucune réponse détectée à ce stade
- **Canal** : email (voie officielle)
- **Interlocuteur** : Florent Boudié, rapporteur du projet constitutionnel relatif à la Corse à l'Assemblée nationale ; copie au secrétariat de la Commission des lois
- **Sujet** : Demande d'audience — contribution associative et citoyenne autour de l'Autonomie de Capacité
- **Niveau de divulgation** : D3 — copie lisible publique avec éléments limités au nécessaire

> Cette copie est une synthèse lisible des faits déjà documentés publiquement, et non une reproduction quasi brute du message. La copie brute reste *à créer* (en attente de la mise au net de l'email réellement envoyé).

## Contexte public

Dans un entretien publié par Corse-Matin le 25 mai 2026, Florent Boudié, rapporteur du projet constitutionnel relatif à la Corse, indiquait vouloir échanger avec :

- les institutions ;
- la Collectivité ;
- l'État ;
- les acteurs socio-économiques ;
- les acteurs associatifs ;
- les acteurs culturels.

Il indiquait également vouloir aller au contact de chacun, y compris de sensibilités non représentées à l'Assemblée de Corse.

## Demande formulée

Le 25 mai 2026, une demande d'audience a été adressée strictement par la voie officielle :

- adresse parlementaire de M. Boudié ;
- copie au secrétariat de la commission des lois ;
- copie à l'Institut Mariani.

Cette demande avait également été annoncée publiquement.

Son objet était de présenter une contribution associative et citoyenne autour de l'**Autonomie de Capacité** :

- distinction entre autonomie institutionnelle et capacité réelle d'agir ;
- place des acteurs associatifs ;
- expérimentation territoriale ;
- approche ouverte, documentée, vérifiable et non partisane.

## Réponse reçue

À ce stade, aucune réponse n'a été détectée dans le fil.

Le statut reste donc : `aucune réponse détectée à ce stade`.

Si la visite en Corse s'achève sans retour, le cas pourra être requalifié en : `demande d'audience non retenue`.

## Point méthodologique

Ce cas ne justifie aucun procès d'intention.

Il met cependant en relation trois faits documentables :

1. une déclaration publique d'ouverture aux acteurs associatifs ;
2. une demande officielle et publiquement annoncée ;
3. aucune continuation détectée à ce stade.

Ces faits interrogent la distance possible entre discours public d'ouverture et actes concrets de coopération.

## Documents liés

- [Tableau de bord](../../dashboard.md)
- [Paquet YAML](../../packets/2026/2026-05-25-demande_audience_boudie.yaml)
- [Copie quasi brute masquée](../../raw/2026/2026-05-25-demande_audience_boudie_redacted.eml.md)

<!-- BEGIN_AUTO: backlinks -->
### Backlinks

*These documents link to this file:*
- [Interaction Packets — Tableau de bord (JHR)](../../dashboard.md)
- [Research Index — Jean Hugues Noël Robert (Profile / Entry Point)](../../../research/index.md)
<!-- END_AUTO: backlinks -->
