---
title: "Copie quasi brute masquée — Courrier public aux parlementaires corses (projet de loi constitutionnelle)"
description: "Near-raw redacted copy (disclosure D3) of the 2026-05-24 public letter sent to the six Corsican parliamentarians on the constitutional bill; cover note + two PDF attachments; no reply at this stage."
author: "Jean Hugues Noël Robert, baron Mariani"
affiliation: "Institut Mariani — émanation R&D de C.O.R.S.I.C.A., 1 cours Paoli, F-20250 Corte, Corsica"
date: "2026-05-24"
status: "journal"
license: "CC BY 4.0"
canonical_url: https://github.com/JeanHuguesRobert/JeanHuguesRobert/blob/main/interaction_packets/raw/2026/2026-05-24-courrier_parlementaires_redacted.eml.md
last_stamped_at: 2026-06-01
---

# Copie quasi brute masquée — Courrier public aux parlementaires corses (projet de loi constitutionnelle)

## Statut

- ID : `2026-05-24-001`
- Niveau de divulgation : `D3`
- Type : copie quasi brute partiellement masquée
- Source : fil Gmail vérifié le 2026-05-28 (le fil ne contient que le message d'envoi ci-dessous)

## Message envoyé

```text
Date: 2026-05-24
From: Jean Hugues Noël Robert <jeanhuguesrobert@gmail.com>
To: michel.castellani@assemblee-nationale.fr (Michel Castellani, député) ;
    francois-xavier.ceccoli@assemblee-nationale.fr (François-Xavier Ceccoli, député) ;
    paul-andre.colombani@assemblee-nationale.fr (Paul-André Colombani, député) ;
    laurent.marcangeli@assemblee-nationale.fr (Laurent Marcangeli, député) ;
    jj.panunzi@senat.fr (J.-J. Panunzi, sénateur) ;
    pt.parigi@senat.fr (P.-T. Parigi, sénateur)
Subject: Courrier public relatif au projet de loi constitutionnelle sur la Corse
Pièces jointes : courrier_public_six_parlementaires_corse.pdf ; note_synthetique_autonomie_capacite_corse.pdf

Messieurs les Sénateurs,
Messieurs les Députés,

Je vous adresse, par vos adresses officielles, le courrier public ci-joint relatif au projet de loi constitutionnelle concernant la Corse.

Une note synthétique accompagne ce courrier afin d'en faciliter l'examen parlementaire.

Le texte source, les références et le suivi public de cette démarche seront également documentés dans un dépôt GitHub ouvert.

Veuillez croire, Messieurs les Sénateurs, Messieurs les Députés, à l'expression de ma considération respectueuse.

Amicalement,

    Jean Hugues

Jean Hugues Noël Robert
Casa Mariani — Institut Mariani
1 cours Paoli
F-20250 Corte
+33 7 53 97 62 87

Travaux ouverts : https://github.com/JeanHuguesRobert
[bloc de signature about.me réduit]
```

## Pièces jointes

- `courrier_public_six_parlementaires_corse.pdf` — le courrier public lui-même ;
- `note_synthetique_autonomie_capacite_corse.pdf` — note synthétique d'accompagnement.

Le contenu des pièces jointes n'est pas transcrit ici. Selon le courrier lui-même, le texte source et les références ont vocation à être documentés dans un dépôt GitHub ouvert.

## Réponse reçue

Aucune réponse reçue à ce stade.

Le fil Gmail, vérifié le 2026-05-28, ne contient que le message d'envoi du 2026-05-24.

## Politique de masquage appliquée

Éléments volontairement masqués ou réduits :

- bloc de signature (image et liens de suivi `about.me`) réduit ;
- contenu des pièces jointes non transcrit.

Éléments conservés (publics) : le nom, l'adresse email, le numéro de téléphone et l'adresse postale de l'auteur (entièrement publics, à sa demande) ; sa qualité ; les noms **et adresses email officielles** des six parlementaires destinataires — données publiques diffusées par les sites de l'Assemblée nationale et du Sénat, s'agissant d'un courrier lui-même explicitement public ; le lien vers les travaux ouverts.

Les coupes sont signalées explicitement.
<!-- BEGIN_AUTO: backlinks -->
### Backlinks

*These documents link to this file:*
- [Interaction lisible — Courrier public aux parlementaires corses (projet de loi constitutionnelle)](../../readable/2026/2026-05-24-courrier_parlementaires.md)
- [Research Index — Jean Hugues Noël Robert (Profile / Entry Point)](../../../research/index.md)
<!-- END_AUTO: backlinks -->
